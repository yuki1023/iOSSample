...
class ViewController: UIViewController, UISearchBarDelegate {
    ...
    func yoho(searchWord: String) {
        let text = "https://api.openweathermap.org/data/2.5/weather?q=\(searchWord)&units=metric&appid=ff6067a853278e206f5cec88436af62e"
        ...
        AF.request(url!, method: .get, parameters: nil, encoding: JSONEncoding.default).responseJSON { (response) in
            ...
            case .success:
                ...
                self.min.text = "\(json["main"]["temp_min"].number!.stringValue)℃"
                self.taikan.text = "\(json["main"]["feels_like"].number!.stringValue)℃"
                self.humidity.text = "\(json["main"]["humidity"].number!.stringValue)%"
                self.wind.text = "\(json["wind"]["speed"].number!.stringValue)m/s"
    ...
    func tenki () {
        //encorde
        let text = "https://api.openweathermap.org/data/2.5/weather?q=chiba&units=metric&appid=ff6067a853278e206f5cec88436af62e".addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
