...
class pantsSaveViewController: UIViewController,UIImagePickerControllerDelegate,UINavigationControllerDelegate  {
    @IBOutlet var pantsImageView: UIImageView!
    
    @IBOutlet var savePants: EMTNeumorphicButton!
    ...
    override func viewDidLoad() {
        ...
        savePants.layer.cornerRadius = 20
        savePants.neumorphicLayer?.depthType = .convex
        savePants.neumorphicLayer?.elementDepth = 10
        savePants.neumorphicLayer?.cornerType = .all
        savePants.neumorphicLayer?.elementBackgroundColor =
               ...
               file.saveInBackground({ (error) in
                   if error != nil {
                    ...
                    let okAction = UIAlertAction(title: "OK", style: .default, handler: { (action) in
                       ...
                       // 画像アップロードが成功
                       let postObject = NCMBObject(className: "savePants")
                       ...
                       postObject?.saveInBackground({ (error) in
                           if error != nil {
                               ...
                               self.pantsImageView.image = nil
                               self.pantsImageView.image = UIImage(named: "photo-placeholder")
